...
def SMM(inparams, T, epshist, extraparams):
    ...
    (q, nu_S, nu_W, kappa, muS, lambd, chiS, eta, xi, mu, \
    ...
    sse = np.sum(np.abs(outarray - np.array([15.83, 0.50])))
    
    print(muW, sig, outarray, sse)
...
nu_W = 4.*q/24.    # decay rate while awake
nu_S = 4.*q/24.    # decay rate while sleeping
kappa = 2.         # curvature of circadian penalty
muW = 3.9245341508783804          # waking asymptote for homeostatic process
...
sig = 0.06313199389331753*.5/0.5601815816719115   # standard deviation of z innovations
...
inparams =np.array([muW, sig])
extraparams = (q, nu_S, nu_W, kappa, muS, lambd, chiS, eta, xi, mu, \
           gamma, yvect, rho)
